...
type KnightMove2DIndex =
	| [FileIndex, RankIndex]
	| [FileIndex, LevelIndex]
...
type KnightPatterns =
	...
	| [-2, 1]
	| [-2, -1];
const knightPatterns: KnightPatterns[] = [
	[1, 2],
	[1, -2],
	[2, 1],
	[2, -1],
...
export function knightMoves3D(pos: Position3D): Position3D[] {
	...
	const frMoves = frPatterns.filter(
		([x, y]) => x >= 1 && x <= 7 && y >= 1 && y <= 7,
	) as KnightMove2DIndex[];
	const frMoves3D: KnightMove3DIndex[] = frMoves.map(([f, r]) => [
		f,
		r,
		levelIndex,
	]);
	...
	const rlMoves = rlPatterns.filter(
		([x, y]) => x >= 1 && x <= 7 && y >= 1 && y <= 7,
	) as KnightMove2DIndex[];
	const rlMoves3D: KnightMove3DIndex[] = rlMoves.map(([r, l]) => [
		fileIndex,
		r,
		l,
	]);
	...
	const flMoves = flPatterns.filter(
		([x, y]) => x >= 1 && x <= 7 && y >= 1 && y <= 7,
	) as KnightMove2DIndex[];
	const flMoves3D: KnightMove3DIndex[] = flMoves.map(([f, l]) => [
		f,
		rankIndex,
		l,
	]);

	const allMoves: KnightMove3DIndex[] = [
		frMoves3D,
		rlMoves3D,
		flMoves3D,
	].flat();

	const allPoisons: Position3D[] = allMoves.map(([f, r, l]) => ({
		file: files[f],
		rank: ranks[r],
		level: levels[l],
	}));
...
export async function makeTour(steps: number, startPos: Position3D) {
	...
	while (tour.length < steps) {
		...
		const futureMovesAvailable = knightMoves3D(nextMove).some(
			(move) =>
				!tour.some((pos) => isSamePosition(pos, move)) &&
				!isSamePosition(move, nextMove),
		);
